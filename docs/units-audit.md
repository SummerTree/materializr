...
|---|---|---|
| comment | src/app/Application.cpp:918 | `// ios_platform.mm).` |
| comment | src/app/Application.cpp:1176 | `// Create a demo box so there's something to see (a 20 mm cube) - but only` |
| comment | src/app/Application.cpp:1961 | `// Absolute linear deflection (mm) and angular deflection (radians). Lower` |
| comment | src/app/Application.cpp:2034 | `// or, from before the presets converted, wrote 1 and reloaded as a 1 mm` |
...
| comment | src/app/Application.cpp:5572 | `// the surface's parametric origin ~0.3 mm off-axis - the fitted anchor` |
| comment | src/app/Application.cpp:5634 | `// 0.64 mm off-axis). Fit a circle to the OUTER wire instead, then` |
| comment | src/app/Application.cpp:5699 | `// then keeps the wrong band, landing 0.6 mm off with a` |
| comment | src/app/Application.cpp:5748 | `// 0.64 mm off-axis at the surface origin) that WON` |
| comment | src/app/Application.cpp:5842 | `// sides at a generous offset (1 mm) so tessellation slack near the` |
| comment | src/app/Application.cpp:5857 | `const double eps = 1.0; // mm` |
| comment | src/app/Application.cpp:6485 | `// lines wrote raw millimetres: placing a dimension under any non-mm unit` |
| comment | src/app/Application.cpp:6520 | `// Hash point positions and circle/arc radii too (quantised to 1e-4 mm)` |
| comment | src/app/Application.cpp:6859 | `// millimetres. 40 mm is a reasonable first view in millimetres and an` |
| comment | src/app/Application.cpp:6919 | `// Measured offsets were 10–50% of a cell. At a 1 mm grid that reads as` |
| comment | src/app/Application.cpp:6920 | `// slightly-fat lines; at 0.1 mm it is most of a cell, i.e. "I can't draw a` |
